...
def index(request):
    ...
    if request.method == 'POST': 
        ...
        try:
            ...
            regressionOutput     = pd.Series(index= data.index, data= regressionOutput    ).rename('predictedScore'      )
            classificationOutput = pd.Series(index= data.index, data= classificationOutput).rename('predictedWeakestLink')

            output = pd.concat([regressionOutput, classificationOutput], axis= 1)

            outputText = output.to_csv()

            return render(
                request= request,
                template_name= 'home.html',
                context= {
                    'outputTable': output.to_dict(orient= 'index'),
                }
            )
...
def downloadFile(request):
    response = HttpResponse(outputText)
    response['Content-Type'] = 'text/plain'
    response['Content-Disposition'] = 'attachment; filename=predictedResults.txt'
    return response
